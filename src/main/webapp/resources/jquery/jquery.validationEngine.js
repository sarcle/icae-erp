 ...
 (function($) {
	 ...
	 var methods = {
		...
		*/
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "condRequired":
						...
						break;
					case "fileExt":
						errorMsg = methods._fileExt(field, rules, i, options);
						break;
		...
		_fileExt : function (field, rules, i, options) {
			var ext = new Array();
			for (var index = i + 1; index < rules.length; index++) {
				if ($.trim(rules[index]) !== "") {
					ext.push(rules[index]);
				}
			}
			
			var extFile = getExtension(field.val());
			var isValid = false;
			
			$.each(ext, function (i, element) {
				if (extFile === element) {
					isValid = true;
					return;
				}
			...
			if (!isValid) {
				return options.allrules.fileExt.alertText
			}
	 ...
	 */
	 $.fn.validationEngine = function(method, divTabs) {
		 ...
		 } else if (typeof(method) == 'string' && method == 'validateTabs') {
			 var validAll = false; 
			 $(divTabs + " > ul > li > a").each(function() {	
				 $(this).click();
				 methods.init.apply(form);
				 validAll = form.validationEngine('validate');
